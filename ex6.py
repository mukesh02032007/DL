...
# Write the frequent words into a file (FIXED: avoided variable shadowing)
with open('output/vocab_frequent.txt', 'w') as output:
    for w_item in vocab_ls:
        output.write(w_item + '\n')
...
if '<s>' in pos_distinct:
    pos_distinct.remove('<s>') # Exclude start token from valid tag sequence predictions
...
with open('output/vocab_frequent.txt', 'r') as vocab_txt:
    for word_item in vocab_txt:
        vocab_frequent.append(word_item.strip('\n'))
...
def viterbi(sentence):
    ...
    for t in range(1, len(sentence)):
        ...
        for p in pos_distinct:
            max_prob = -1
            prev_state = pos_distinct[0]  # FIXED: Default fallback prevents KeyError
            for p_prev in pos_distinct:
                prob = V[t-1][p_prev] * transition.get(p + '|' + p_prev, 0) * emission.get(sentence[t] + '|' + p, 0)
                if prob > max_prob:
                    max_prob = prob
                    prev_state = p_prev
            V[t][p] = max_prob
